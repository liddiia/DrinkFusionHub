...
export function renderFavPagination(cocktailArr) {
  ...
  console.log('fav: ', sortedCardsArr);

  if (sortedCardsArr[0].length === 0 || !sortedCardsArr[0]) {
    haventAddedFavoriteCoctails(contNoCoctails, titleGalleryEl);
  } else {
    sortedCardsArr[0].forEach(item => {
      renderADrink(item);
      console.log(item);
    });
  }
  ...
  function onPaginationBtnClick(evt) {
    ...
    if (sortedCardsArr[currentPageIndex].length === 0 || !sortedCardsArr[currentPageIndex]) {
      haventAddedFavoriteCoctails(contNoCoctails, titleGalleryEl);
    } else {
      sortedCardsArr[currentPageIndex].forEach(item => {
        renderADrink(item);
        console.log(item);
      });
    }
